...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'inscription') {
        ...
        $stmt = $pdo->prepare("INSERT INTO etudiants (prenom, nom, ine, email, telephone, genre, password) VALUES (?, ?, ?, ?, ?, ?, ?)");
        if ($stmt->execute([$prenom, $nom, $ine, $email, $telephone, $genre, $password])) {
            header("Location: ../PAGES/etudiant.html");
            exit;
        } else {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            header("Location: ../PAGES/etudiantForm.?error=invalid_credentials");
            exit;
        }
    }
}
